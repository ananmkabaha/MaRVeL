...
def handle_relu(model,var_list, affine_counter, num_neurons, lbi, ubi, relu_groupsi, use_milp, partial_milp_neurons,grad_layers=[]):
    use_milp = use_milp
    #print("relu groups ")

    start = len(var_list)
    relu_counter = start
    #print("in handle_relu")
    #print(partial_milp_neurons)
    #print(grad_layers)
    cross_over_idx = list(np.nonzero(np.array(lbi)*np.array(ubi)<0)[0])
    width = np.array(ubi) - np.array(lbi)
    #width = np.random.rand(len(ubi))
    if len(grad_layers)> 0:
        cross_over_idx = sorted(cross_over_idx, key= lambda x: -width[x])
        #cross_over_idx = sorted(cross_over_idx, key= lambda x: -np.absolute(width*grad_layers)[x])
    else:
        cross_over_idx = sorted(cross_over_idx, key= lambda x: -width[x])
    #print(width)
    #print(cross_over_idx)
    #print(width)
    ...
    #print(milp_encode_idx)
    #print("len(milp_encode_idx):",len(milp_encode_idx))
...
def create_model(nn, LB_N0, UB_N0, nlb, nub, relu_groups, numlayer, use_milp,is_nchw=False, partial_milp=0, max_milp_neurons=-1,grad_layers = []):
    model = Model("milp")
    ...
    for i in range(numlayer):
        if nn.layertypes[i] in ['SkipCat']:
            ...
            grad_layer = []
            if len(grad_layers)>0:
                grad_layer = grad_layers[activation_counter]
            index = nn.predecessors[i+1][0]
            partial_milp_neurons = (first_milp_layer <= i) * (max_milp_neurons if max_milp_neurons >= 0 else len(nlb[i]))
            #print(partial_milp_neurons)
            if relu_groups is None:
                counter = handle_relu(model, var_list, counter, len(nlb[i]), nlb[index-1], nub[index-1], [], use_milp, partial_milp_neurons,grad_layer)
            else:
                counter = handle_relu(model, var_list, counter, len(nlb[i]), nlb[index-1], nub[index-1], relu_groups[activation_counter], use_milp, partial_milp_neurons,grad_layer)
...
def verify_network_with_milp(nn, LB_N0, UB_N0, nlb, nub, constraints, max_milp_neurons=-1,time_limit_=10,is_minimize=True, spatial_constraints=None, is_nchw=False):
    #max_milp_neurons = 15
    ...
    counter, var_list, model = create_model(nn, LB_N0, UB_N0, nlb, nub, None, numlayer, use_milp=True, is_nchw=is_nchw,
                                            partial_milp=-1, max_milp_neurons= max_milp_neurons)
    ...
    for or_list in constraints:
        ...
        for is_greater_tuple in or_list:
            #print(or_list)
            #print (is_greater_tuple)
            (i__, j__, k__) = is_greater_tuple
            ...
            for cutoff in [GRB.INFINITY]:
                model.reset()
                model.setParam(GRB.Param.TimeLimit,time_limit_)
            ...
            #print("model.solcount:",model.solcount)
            ...
            '''
            print("len_mexn",sum(model.xn[0:input_size]),"len_me",sum(model.x[0:input_size]))
            print("len_mexn",np.sum(np.array(model.xn[0:input_size])-np.array(model.x[0:input_size])))
            model.setParam('SolutionNumber',1)
            print("setParam",np.sum(np.array(model.xn[0:input_size])-np.array(model.x[0:input_size])))
            ...
            if model.Status == 6 or model.objbound > 0:
                ...
                if model.solcount > 0:
                    if model.solcount == 1:
                        ...
                        non_adv_target.append(j__)
                    else:
                        for i_a in range(solution_pool_size):
                            model.setParam('SolutionNumber',i_a)
                            non_adv_examples.append(model.xn[0:input_size])
                            non_adv_val.append(model.objval)
                            non_adv_target.append(j__)

                    '''
                    print(non_adv_val)
                    print(sum(non_adv_examples[0]))
                    print(sum(non_adv_examples[1]))
                    print(sum(non_adv_examples[2]))
                    '''
                    
                    #print(model.x[0:input_size])
                    #imgplot = plt.imshow(np.array(model.x[0:input_size]).reshape(28,28), cmap='gray', vmin=0, vmax=1)
                    #plt.show()
                break
            elif model.solcount > 0:
                #adv_examples.append(model.x[0:input_size])
                #adv_val.append(model.objval)
                #adv_target.append(j__)
                ...
                if model.solcount == 1:
                    ...
                    for i_a in range(solution_pool_size):
                        model.setParam('SolutionNumber',i_a)
                        adv_examples.append(model.xn[0:input_size])
                        adv_val.append(model.objval)
                        adv_target.append(j__)
        ...
        if not or_result:
            if len(adv_examples) > 0:
                return False, adv_examples, adv_val,adv_target
            else:
                return False, None, None,None
    if len(non_adv_examples) > 0:
        return True, non_adv_examples, non_adv_val, non_adv_target
    else:
